...
function AddWeight() {
  ...
  const editItem = (id)=>{
   fetchDataFromApi(`/api/product/productWEIGHT/${id}`)
   .then((res)=>{
   ...
   if (formFields.name === '') {
    ...
    return;
     ...
     if (!editId) {
      postData('/api/product/productWEIGHT/add', formFields, { withCredentials: true })
      .then((res) => {
        if (res?.error === false) {
          context?.openAlertBox('success', res?.message);
          setFormFields({ name: '' });
       ...
       // Edit existing RAM
       profileEditData(`/api/product/updateProductWEIGHT/${editId}`, formFields, { withCredentials: true })
        .then((res) => {
          if (res?.error === false) {
          context?.openAlertBox('success', res?.message);
          setFormFields({ name: '' });

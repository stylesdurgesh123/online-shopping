...
function AddRAMS() {
  ...
  const editItem = (id)=>{
   fetchDataFromApi(`/api/product/productRAMS/${id}`)
   .then((res)=>{
   ...
   if (formFields.name === '') {
    ...
    return;
     ...
     if (!editId) {
      postData('/api/product/productRAMS/add', formFields, { withCredentials: true })
      .then((res) => {
        if (res?.error === false) {
          context?.openAlertBox('success', res?.message);
          setFormFields({ name: '' });
       ...
       // Edit existing RAM
       profileEditData(`/api/product/updateProductRAMS/${editId}`, formFields, { withCredentials: true })
        .then((res) => {
          if (res?.error === false) {
          context?.openAlertBox('success', res?.message);
          setFormFields({ name: '' });

...
function EditAddProduct() {
  ...
  const [productSubCat, setProductSubtCat] =useState('');
  ...
  const context=useContext(MyContext);
   ...
   useEffect(() => {
     ...
     fetchDataFromApi(`/api/product/${context.isOpenFullScreenPanel.id}`)
    .then((res) => {
      setFormFields({   
        ...
        sale: res.product.sale,
      });

      setProductCat(res.product.catId);
      setProductSubtCat(res.product.subCatId);
 ...
 const selectCatByName=(name)=>{
  ...
  const handleChangeProductSubCat = (event) => {
    setProductSubtCat(event.target.value);
 ...
 const handleSubmit=(e)=>{
  ...
  setIsLoading(true)
  profileEditData(`/api/product/updateProduct/${context.isOpenFullScreenPanel.id}`,formFields,{withCredentials: true})
  .then((res)=>{
  if(res?.error===false){
